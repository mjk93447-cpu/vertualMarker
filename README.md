...
- **버전 번호**: 현재 배포 버전은 `v6.0.0` 이다.

### ver8 신뢰성/피드백 업데이트

- **다중 긴 선 시각화**: 거북이선 외의 상위 긴 선들을 희미한 회색으로 함께 표시하여 형상 컨텍스트를 제공한다.
- **핵심 좌표+오차 시각화 강화**:
  - Mv, Mv', BSP 좌표를 이미지에 직접 표기한다.
  - Mv'→BSP 벡터를 화살표로 표시하고, `dX`, `dY`, `distance`를 오버레이한다.
  - 이 값은 초기 오차(align error) 진단의 핵심 지표로 사용한다.
- **자동 보정 및 즉시 피드백**:
  - 작은 선 끊어짐(근접 끝점) 발생 시 bridge point를 삽입해 자동 보정한다.
  - FH/UH strict 탐색 실패 시 임계값 완화(단계적)로 재탐색한다.
  - 분기/애매한 구간은 diagnostics 경고로 로그와 이미지에 즉시 노출한다.
- **운영성 강화 (GUI)**:
  - `Max Heal Gap (pixel)` 파라미터로 선 끊어짐 보정 민감도를 조정한다.
  - 실행 중 Progress Bar, Runtime Status, 파일별 성공/실패 색상 피드백을 제공한다.
  - 첫 실패 발생 시 즉시 경고 팝업을 표시한다.
  - `Cancel` 버튼으로 대량 배치 실행을 중단할 수 있다.
  - 마지막 시각화 이미지를 GUI 내 Preview로 즉시 확인할 수 있다.
- **입력 품질 피드백**:
  - 파싱 시 malformed line 개수를 집계해 로그로 즉시 보고한다.
- **버전 번호**: 현재 배포 버전은 `v8.0.0` 이다.

### ver8 전문가 평가 라운드(3회)

- **Round 1**: 장시간 배치 사용성, 즉시 실패 피드백, line-break 튜닝 파라미터 요구사항 식별
- **Round 2**: 파싱 품질 보고와 취소 UX 보완사항 식별
- **Round 3**: blocker 없음 판정, 운영 배포 Go 평가
- **최종 평가 점수**: `88/100` (mixed-quality data 운영 기준)
